...
window.XRAY_Renderer = (() => {
  ...
  function buildTree(data, options = {}) {
    const rootPath = options.rootPath || 'response';
    const root = document.createElement('div');
    root.className = 'xr-tree-root';
    _node(root, data, null, 0, rootPath, options);
    return root;
  }

  function _node(parent, val, key, depth, path, options) {
    ...
    if (isComplex) {
      ...
      line.setAttribute('data-xr-path', path);
      line.addEventListener('click', () => {
        if (typeof options.onPathSelect === 'function') options.onPathSelect(path, line);
      });
      ...
      function renderChildren() {
        ...
        keys.forEach((k, i) => {
          const childPath = type === 'array'
            ? `${path}[${i}]`
            : `${path}.${k}`;
          _node(children, val[k], type === 'array' ? i : k, depth + 1, childPath, options);
        });
