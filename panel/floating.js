...
.xr-console-method.xr-m-get { background: var(--xr-blue); color: var(--xr-bg); }
.xr-console-method.xr-m-post { background: var(--xr-green); color: var(--xr-bg); }
.xr-console-method.xr-m-put { background: var(--xr-yellow); color: var(--xr-bg); }
.xr-console-method.xr-m-delete { background: var(--xr-red); color: var(--xr-bg); }
.xr-console-method.xr-m-patch { background: var(--xr-purple); color: var(--xr-bg); }
.xr-console-url { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.xr-console-status { font-weight: 600; }
.xr-console-status.xr-s2 { color: var(--xr-green); }
.xr-console-status.xr-s4 { color: var(--xr-yellow); }
.xr-console-status.xr-s5 { color: var(--xr-red); }
.xr-console-dur { color: var(--xr-muted); }
...
.xr-console-entry-cmd::before {
  content: '›';
  color: var(--xr-accent);
  font-weight: bold;
}
.xr-console-entry-result { color: var(--xr-text); }
.xr-console-entry-result.xr-type-string { color: var(--xr-green); }
.xr-console-entry-result.xr-type-number { color: var(--xr-blue); }
.xr-console-entry-result.xr-type-boolean { color: var(--xr-purple); }
.xr-console-entry-result.xr-type-null,
.xr-console-entry-result.xr-type-undefined { color: var(--xr-muted); font-style: italic; }

.xr-console-entry.xr-error {
  background: rgba(248, 113, 113, 0.08);
  border-color: var(--xr-red);
}
.xr-console-entry.xr-error .xr-console-entry-result { color: var(--xr-red); }

.xr-console-entry.xr-assertion { }
.xr-console-entry.xr-assertion.xr-pass {
  background: rgba(74, 222, 128, 0.08);
  border-color: var(--xr-green);
}
.xr-console-entry.xr-assertion.xr-pass .xr-console-entry-result { color: var(--xr-green); }
.xr-console-entry.xr-assertion.xr-pass .xr-console-entry-result::before { content: '✓ '; }
.xr-console-entry.xr-assertion.xr-fail {
  background: rgba(248, 113, 113, 0.08);
  border-color: var(--xr-red);
}
.xr-console-entry.xr-assertion.xr-fail .xr-console-entry-result { color: var(--xr-red); }
.xr-console-entry.xr-assertion.xr-fail .xr-console-entry-result::before { content: '✗ '; }

.xr-console-input-bar {
  display: flex;
  flex-direction: column;
  border-top: 1px solid var(--xr-border);
  background: var(--xr-bg2);
}
.xr-console-cheatsheet {
  padding: 6px 12px;
  font-family: 'JetBrains Mono', 'Fira Code', monospace;
  font-size: 10px;
  color: var(--xr-muted);
...
.xr-console-cheatsheet code {
  background: var(--xr-bg3);
  padding: 1px 4px;
  border-radius: 3px;
  margin-right: 6px;
}
.xr-console-input-row {
  display: flex;
...
.xr-console-input::placeholder { color: var(--xr-muted); }
.xr-console-run {
  padding: 8px 16px;
  margin: 8px;
  background: var(--xr-accent);
...
.xr-console-run:hover { opacity: 0.85; }
    `;
  }
...
<div class="xr-body">
  <div class="xr-list-wrap" id="xr-list-wrap">
  ...
  <div class="xr-console-pane" id="xr-console-pane">
    <div class="xr-console-context" id="xr-console-context">
      <span class="xr-console-context-empty">Select a request to set $r context</span>
    </div>
    <div class="xr-console-output" id="xr-console-output"></div>
    <div class="xr-console-input-bar">
      <div class="xr-console-cheatsheet">
        <code>$r</code><code>$res</code><code>$req</code><code>$all()</code><code>$curl</code><code>copy()</code><code>pin()</code><code>assert()</code><code>_.</code>
      </div>
      <div class="xr-console-input-row">
        <textarea class="xr-console-input" id="xr-console-input" placeholder="$res.users.map(u => u.email)  ·  Ctrl+Enter to run" rows="1"></textarea>
        <button class="xr-console-run" id="xr-console-run">▶ Run</button>
      </div>
    </div>
  </div>
</div>
...
<div class="xr-settings-backdrop" id="xr-settings-backdrop">
  ...
  function _selectEntry(id) {
    ...
    // Update console context when selection changes
    if (changed) _updateConsoleContext();
  }
...
func main() {
  ...
  function _updateConsoleContext() {
    if (!_dom.consoleContext) return;
    const entry = _state.selectedId ? _state.entries.find(e => e.id === _state.selectedId) : null;
    
    if (window.XRAY_Console) {
      window.XRAY_Console.setContext(entry);
    }
    
    if (!entry) {
      _dom.consoleContext.innerHTML = '<span class="xr-console-context-empty">Select a request to set $r context</span>';
      return;
    }
    
    const method = (entry.method || 'GET').toUpperCase();
    const url = entry.urlPath || entry.url || '(unknown)';
    const status = entry.status || 0;
    const duration = entry.duration || 0;
    const sClass = status >= 200 && status < 300 ? 'xr-s2' : status >= 400 && status < 500 ? 'xr-s4' : status >= 500 ? 'xr-s5' : '';
    
    _dom.consoleContext.innerHTML = `
      <span class="xr-console-method xr-m-${method.toLowerCase()}">${method}</span>
      <span class="xr-console-url">${url}</span>
      <span class="xr-console-status ${sClass}">${status}</span>
      <span class="xr-console-dur">${duration}ms</span>
    `;
  }

  async function _runConsoleCode() {
    const input = _dom.consoleInput;
    if (!input) return;
    ...
    if (!window.XRAY_Console) {
      console.error('[XRAY] Console module not loaded');
      return;
    }
    
    const result = await window.XRAY_Console.execute(code);
    _renderConsoleResult(code, result);
    input.value = '';
    input.style.height = 'auto';
  }

  function _renderConsoleResult(code, result) {
    const output = _dom.consoleOutput;
    if (!output) return;
    
    const entry = document.createElement('div');
    entry.className = 'xr-console-entry';
    
    // Command line
    const cmdDiv = document.createElement('div');
    cmdDiv.className = 'xr-console-entry-cmd';
    cmdDiv.textContent = code.length > 100 ? code.slice(0, 100) + '…' : code;
    entry.appendChild(cmdDiv);
    
    // Result
    const resDiv = document.createElement('div');
    resDiv.className = 'xr-console-entry-result';
    
    if (result.type === 'empty') {
      return; // Don't render empty
    } else if (result.type === 'error') {
      entry.classList.add('xr-error');
      resDiv.textContent = result.error?.message || 'Unknown error';
    } else if (result.type === 'assertion') {
      entry.classList.add('xr-assertion', result.pass ? 'xr-pass' : 'xr-fail');
      resDiv.textContent = result.message;
    } else if (result.type === 'undefined') {
      resDiv.classList.add('xr-type-undefined');
      resDiv.textContent = 'undefined';
    } else if (result.type === 'null') {
      resDiv.classList.add('xr-type-null');
      resDiv.textContent = 'null';
    } else if (result.type === 'string') {
      resDiv.classList.add('xr-type-string');
      resDiv.textContent = `"${result.result}"`;
    } else if (result.type === 'number' || result.type === 'boolean') {
      resDiv.classList.add(`xr-type-${result.type}`);
      resDiv.textContent = String(result.result);
    } else if (result.type === 'array' || result.type === 'object') {
      // Use renderer for complex types
      if (window.XRAY_Renderer) {
        const tree = window.XRAY_Renderer.buildTree(result.result, { rootPath: 'result' });
        resDiv.appendChild(tree);
      } else {
        resDiv.textContent = JSON.stringify(result.result, null, 2);
      }
    } else {
      resDiv.textContent = String(result.result);
    }
  ...
  function _bindConsoleEvents() {
    if (!_dom.consoleInput || !_dom.consoleRun) return;
    
    // Run button
    _dom.consoleRun.addEventListener('click', _runConsoleCode);
    
    // Ctrl+Enter to run, Arrow keys for history
    _dom.consoleInput.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowUp' && _dom.consoleInput.selectionStart === 0) {
        e.preventDefault();
        const prev = window.XRAY_Console?.navigateHistory('up');
        if (prev !== null) _dom.consoleInput.value = prev;
        return;
      }
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (next !== null) _dom.consoleInput.value = next;
        return;
      }
    });
    
    // Auto-resize textarea
    _dom.consoleInput.addEventListener('input', () => {
      _dom.consoleInput.style.height = 'auto';
      _dom.consoleInput.style.height = Math.min(_dom.consoleInput.scrollHeight, 150) + 'px';
    });
    
    // Init console module
    if (window.XRAY_Console?.init) {
  ...
  function _bindEvents() {
    ...
    _root.querySelectorAll('.xr-tab').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (isConsole) {
          // Update console context when switching to console tab
          _updateConsoleContext();
          _dom.consoleInput?.focus();
        } else {
          _state.selectedId = null;
          _rebuildList();
          _renderDetail(null);
  ...
  const _public = {
    ...
    async init(options = {}) {
      ...
      // Console refs
      _dom.listWrap         = getById('xr-list-wrap');
      _dom.consolePane      = getById('xr-console-pane');
      _dom.consoleContext   = getById('xr-console-context');
      _dom.consoleOutput    = getById('xr-console-output');
      _dom.consoleInput     = getById('xr-console-input');
      _dom.consoleRun       = getById('xr-console-run');
